...
const CELL = 'border border-neutral-300 text-center';
...
export default function TimelineCalendar({
    ...
    return (
      <div className="p-8 text-center sm:p-12">
        ...
        <button
          type="button"
          onClick={onAddTask}
          className="rounded bg-neutral-900 px-4 py-2 text-xs font-semibold text-white hover:bg-neutral-700"
        >
          + Add task
        </button>
      </div>
  ...
  return (
    ...
    <div
      id="timeline-print"
      className={`bg-white p-3 sm:p-6 ${fitWidth ? '' : 'min-w-[680px]'} lg:min-w-0`}
    >
      ...
      {blocks.map((block) => (
        <section key={block.key} className="mb-8 sm:mb-10">
          <h2 className="mb-3 text-center text-lg font-bold tracking-wide text-neutral-900 sm:text-xl">
            {block.label}
          </h2>
                ...
                {week.days.map((day, i) => (
                  <div
                    ...
                    className={`${CELL} ${dayText} py-1 ${
                      day.date && day.isOff ? 'bg-[#1F4E5A] text-white' : 'text-neutral-800'
                    ...
                    .flatMap((p) =>
                      p.segments.map((seg, si) => (
                        <button
                          ...
                          className={`flex h-6 items-center justify-center overflow-hidden border border-neutral-300 px-0.5 font-bold leading-none sm:px-1 ${blockText} ${
